            ...
            >
              ...
              <ConfirmToggle
                ...
                onChanged={newStatus => {
                  ...
                  setReservations(list => list.map(r => r.id === selected.id ? { ...r, status: newStatus } : r))
                }}
              />
...
function ConfirmToggle({
  ...
  async function set(next: "confirmed" | "pending") {
    setSaving(next === "confirmed" ? "yes" : "no")
    setError(null)
    try {
        ...
        const msg =
          typeof raw === "string"
            ...
            : raw && typeof raw === "object"
              ? (raw.message || raw.hint || raw.details || raw.code || JSON.stringify(raw))
              : `HTTP ${res.status}`
        setError(String(msg))
      }
    } catch (e) {
    ...
    <div className="mt-5 pt-4" style={{ borderTop: "1px solid #ede9e0" }}>
      ...
      <div className="flex gap-2">
        <button
          ...
          disabled={!!saving}
          className="flex-1 text-xs font-semibold px-3 py-2 rounded"
          style={{
            background: isConfirmed ? "#4a6b3a" : "#fffefb",
            color: isConfirmed ? "#fffefb" : "#36342e",
            border: isConfirmed ? "1px solid #4a6b3a" : "1px solid #c5c0b1",
            cursor: saving ? "default" : "pointer",
            opacity: saving === "no" ? 0.5 : 1,
            transition: "background 150ms, color 150ms, border-color 150ms",
          }}
        >
          {saving === "yes" ? "…" : `✓ ${lang === "de" ? "Ja" : "Yes"}`}
        </button>
        <button
          type="button"
          onClick={() => set("pending")}
          ...
          className="flex-1 text-xs font-semibold px-3 py-2 rounded"
          style={{
            background: !isConfirmed ? "#6b1535" : "#fffefb",
            color: !isConfirmed ? "#fffefb" : "#36342e",
            border: !isConfirmed ? "1px solid #6b1535" : "1px solid #c5c0b1",
            cursor: saving ? "default" : "pointer",
            opacity: saving === "yes" ? 0.5 : 1,
            transition: "background 150ms, color 150ms, border-color 150ms",
          }}
        >
          {saving === "no" ? "…" : lang === "de" ? "Nein" : "No"}
        </button>
      </div>
      {error && (
